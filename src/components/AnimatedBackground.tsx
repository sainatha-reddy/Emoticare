...
const AnimatedBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Create 40-50 random shapes (more visible)
    const shapeCount = Math.floor(Math.random() * 10) + 40;
    
    for (let i = 0; i < shapeCount; i++) {
      const type = shapeTypes[Math.floor(Math.random() * shapeTypes.length)];
      const color = colors[Math.floor(Math.random() * colors.length)];
      const size = Math.random() * 20 + 10; // 10-30px (larger for visibility)
      ...
      const opacity = Math.random() * 0.4 + 0.3; // 0.3-0.7 opacity (higher minimum)
